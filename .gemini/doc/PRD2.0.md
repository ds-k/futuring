...
## 2. MVP 기술 스택 및 채택 사유 (채용 과제 및 단기 완성을 위한 전략적 축약)

- **프론트엔드**: **Next.js** 
  - *채택 사유*: 타겟 회사의 스택(Next.js/React 기반 Webview)과 완벽히 일치하며, Vercel 등을 통한 즉각적인 배포가 가능하여 단기 완성에 최적화되어 있습니다.
- **백엔드**: **NestJS** 
  - *채택 사유*: 타겟 회사의 백엔드 스택(NestJS)을 반영했습니다. 기존 원본 PRD의 OpenFaaS/Lambda 기반 분산 처리를 단일 서버 구조로 축약하여 개발 속도와 과제 완성도를 극대화했습니다.
- **메시지 큐**: **BullMQ + Redis** 
...
- **데이터베이스**: **PostgreSQL (AWS RDS)** 
  - *채택 사유*: 타겟 회사의 DB(PostgreSQL) 및 인프라(AWS EKS 등) 환경에 부합함을 증명하기 위해, 낯선 NoSQL(ScyllaDB) 대신 RDS를 채택하여 실무 투입 가능성을 어필합니다.
